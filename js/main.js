...
document.addEventListener('DOMContentLoaded', () => {
  // 1. Theme Management (Dark Obsidian by default, Luxury Warm Light option)
  const themeToggleBtn = document.getElementById('theme-toggle-btn');
  const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
  const savedTheme = localStorage.getItem('murdifin_theme') || (prefersDark ? 'dark' : 'dark'); // Default to dark luxury
  ...
  if (themeToggleBtn) {
    themeToggleBtn.addEventListener('click', () => {
      ...
      localStorage.setItem('murdifin_theme', newTheme);
      updateThemeIcon(newTheme);
      showToast(`Mode ${newTheme === 'dark' ? 'Obsidian Gelap' : 'Terang Elegan'} aktif`);
    });
  }
  ...
  function updateThemeIcon(theme) {
    ...
    if (theme === 'light') {
      ...
      `;
      themeToggleBtn.setAttribute('title', 'Beralih ke Obsidian Dark Mode');
    } else {
      themeToggleBtn.innerHTML = `
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      `;
      themeToggleBtn.setAttribute('title', 'Beralih ke Light Mode');
    }
  }
